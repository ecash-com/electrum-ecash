...
PACKAGE="Electrum eCash"  # ECX (keep in sync with pyinstaller.spec PACKAGE_NAME)
GIT_REPO=https://github.com/ecash-com/electrum-ecash
...
hdiutil create -fs HFS+ -volname "$PACKAGE" -srcfolder "dist/$PACKAGE.app" \
    "dist/electrum-ecash-$VERSION-unsigned.dmg" || fail "Could not create .DMG"  # ECX: quote, PACKAGE has a space
