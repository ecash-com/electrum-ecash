...
(
    ...
    # ECX: setuptools derives the sdist name from setup.py's `name`, which is no
    # longer "Electrum". Ask it rather than hardcoding, or the mv below fails.
    SDIST_BASENAME=$(python3 setup.py --fullname)
    if ([ "$OMIT_UNCLEAN_FILES" = 1 ]); then
        FINAL_DISTNAME="$SDIST_BASENAME-sourceonly.tar.gz"
    else
        FINAL_DISTNAME="$SDIST_BASENAME.tar.gz"
    fi
    if ([ "$OMIT_UNCLEAN_FILES" = 1 ]); then
        mv "$PY_DISTDIR/$SDIST_BASENAME.tar.gz" "$PY_DISTDIR/../$FINAL_DISTNAME"
        rmdir "$PY_DISTDIR"
    fi
    ...
    find -exec touch -h -d '2000-11-11T11:11:11+00:00' {} +
    GZIP=-n tar --sort=name -czf "$FINAL_DISTNAME" "$SDIST_BASENAME/"
